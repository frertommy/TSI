...
// Load Opta data from JSON files (generated by seed script)
...
async function getTeamData(id: string) {
  ...
  // Fetch Opta history: try DB, fall back to JSON
...
export default async function TeamDetailPage({ params }: PageProps) {
  ...
  const { team, history, optaHistory, stats, matches, leagueRank } = data;
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8 md:py-12">
      ...
      {team.optaRating != null && (
        <div className="flex gap-4 mb-6 flex-wrap">
          <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg px-4 py-3">
            <div className="text-xs text-[var(--text-dim)] mb-1">Opta Power Rating</div>
            <div className="text-lg font-bold text-[#f59e0b] tabular-nums">
              {team.optaRating.toFixed(1)}
            </div>
          </div>
          {team.optaRank != null && (
            <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg px-4 py-3">
              <div className="text-xs text-[var(--text-dim)] mb-1">Opta World Rank</div>
              <div className="text-lg font-bold text-[#f59e0b] tabular-nums">
                #{team.optaRank}
              </div>
            </div>
      ...
      <section className="mb-8">
        ...
        <TSIChart history={history} optaHistory={optaHistory} />
      </section>
      ...
      <footer className="mt-16 pt-8 border-t border-[var(--card-border)] text-center text-xs text-[var(--text-dim)]">
        <p>
          TSI Oracle v1.0 &middot; Data from ClubElo + Opta &middot; Engine: Elo + Sigmoid Mapping (μ=1850, σ=120)
        </p>
      </footer>
    </main>
  );
